...
- 캐시/세션: `spring-boot-starter-data-redis`
...
## 환경변수 / 실행
- 설정값은 `application.yaml`의 `${...}` 플레이스홀더 → **`server/.env`** 에서 주입(`spring.config.import: optional:file:.env[.properties]`). CI/운영은 실제 OS 환경변수로 덮어씀.
- 최초 1회: `cp .env.example .env` (server/ 에서). 실 시크릿은 `.env`(gitignore)에만, `.env.example`엔 플레이스홀더만.
- **로컬 DB(PostgreSQL)**: `docker compose up -d db` (`docker-compose.yml`, `.env` 값 사용). 종료 `docker compose down`(데이터까지 `-v`).
- 빌드/실행: `./gradlew bootRun` (DB 떠 있어야 정상 기동; `ddl-auto: validate` + Flyway 스키마 전제).
- 테스트: `./gradlew test` (외부 의존 없이 슬라이스/H2). DB/Redis 불필요.
