 ...
 *
 * 토큰 민감도: [accessToken]/[refreshToken] 은 민감 정보다. DB 저장 시 [CalendarTokenEncryptionConverter]
 * (AES-256-GCM, ADR-0025)로 컬럼 암호화한다(키 미설정 시 평문 폴백). 응답 DTO 에는 절대 토큰을
 * 싣지 않는다(presentation 계층에서 제외).
 *
...
class CalendarConnection(
    ...
    /** provider access token(민감, 저장 시 AES-GCM 암호화). null 이면 미발급/스텁. */
    @Convert(converter = CalendarTokenEncryptionConverter::class)
    @Column(name = "access_token", length = 4096)
    var accessToken: String? = null,

    /** provider refresh token(민감, 저장 시 AES-GCM 암호화). */
    @Convert(converter = CalendarTokenEncryptionConverter::class)
